...
class Microservice1:
    def send_request_to_mediator(self, mediator):
        ...
        print(f"\r\nMicroserviciul #1 a primit un răspuns de la mediator: {response}")


class Microservice2:
    def process_request(self, request):
        print(f"\r\nMicroserviciul #2 a primit o cerere de la mediator: {request}")
        time.sleep(2)
...
class Microservice3:
    def process_request(self, request):
        print(f"\r\nMicroserviciul #3 a primit o cerere de la mediator: {request}")
        time.sleep(1)
...
class Mediator:
    ...
    def process_request(self, request):
        print(f"\r\nMediatorul a primit o cerere: {request}")

        with ThreadPoolExecutor(max_workers=2) as executor:
            future1 = executor.submit(self.microservice2.process_request, request)
            future2 = executor.submit(self.microservice3.process_request, request)

        response_from_microservice2 = future1.result()
        response_from_microservice3 = future2.result()
